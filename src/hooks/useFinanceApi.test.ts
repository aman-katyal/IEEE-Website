...
describe("useFinanceApi Hook Suite", () => {
  ...
  it("records cash dues with secure random ID", async () => {
    const { result } = renderHook(() => useFinanceApi());
    await act(async () => {
      const outcome = await result.current.recordCashDues({
        studentName: "John Doe",
        purdueEmail: "johndoe@purdue.edu",
        amountPaid: 20,
      });
      expect(outcome.success).toBe(true);
      expect(outcome.dues?.id).toMatch(
        /^dues-cash-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/,
      );
    });
