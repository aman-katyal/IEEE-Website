  ...
  const handleParseCsv = (content: string, filename: string) => {
    ...
    try {
      ...
      dataLines.forEach((line, idx) => {
        ...
        if (parts.length >= 2) {
          ...
          parsedRecords.push({
            ...
            paymentMethod: ["TooCOOL", "Cash", "Card"].includes(method)
              ? method
              : "TooCOOL",
      ...
      if (parsedRecords.length === 0) {
        setImportError(
          "Could not parse any valid student dues records from CSV.",
        );
        return;
      }
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  aria-label={
                    copiedCOOL
                      ? "Copied text"
                      : "Copy Formatted Purdue COOL Batch Text"
                  }
        ...
        <DialogContent className="max-w-2xl bg-[#121214] text-slate-100 border border-slate-700/80 shadow-2xl p-6 overflow-y-auto max-h-[90vh]">
          ...
          <div className="space-y-4 py-2">
            ...
            >
              ...
              <p className="text-[11px] text-slate-500 mt-1">
                Accepts standard TooCOOL CSV format (Student Name, Purdue Email,
                Amount, Method, Semester)
              </p>
            </div>
              ...
              <div className="space-y-2 p-3 bg-slate-900/90 border border-slate-800 rounded-lg">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-semibold text-white">
                    Parsed {importedCsvData.length} records from{" "}
                    {importFileName}
                  </span>
