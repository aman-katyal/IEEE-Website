...
export function TreasurerFinanceView({
  ...
  const matrixData = useMemo(() => {
    // ⚡ Bolt: Pre-group child arrays into a Map to avoid O(N*M) nested filtering bottleneck
    const purchasesByCommittee = new Map<string, typeof purchases>();
    for (const p of purchases) {
      const existing = purchasesByCommittee.get(p.committeeId) ?? [];
      existing.push(p);
      purchasesByCommittee.set(p.committeeId, existing);
    }

    const inflowsByCommittee = new Map<string, typeof fundingInflows>();
    for (const inf of fundingInflows || []) {
      const existing = inflowsByCommittee.get(inf.committeeId) ?? [];
      existing.push(inf);
      inflowsByCommittee.set(inf.committeeId, existing);
    }
