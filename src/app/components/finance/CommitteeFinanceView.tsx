...
export function CommitteeFinanceView({
  ...
  const [isCashDuesModalOpen, setIsCashDuesModalOpen] =
  ...
  const handleSubmitPurchase = async (e: React.FormEvent) => {
    ...
    if (
      ...
      setFormError(
        "Please fill out all required address fields (Street, City, State, ZIP).",
      );
      return;
    }
    ...
    try {
      ...
      if (result && result.success === false) {
        setFormError(
          result.error ||
            "Failed to submit purchase request. Please check your inputs.",
        );
            ...
            {duesSearchResults.length > 0 ? (
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-amber-400 py-1 px-2">
                <div className="flex items-center gap-2">
                  ...
                  <span>No matching dues record found for "{duesQuery}".</span>
                </div>
                <Button
                  size="sm"
                  variant="ghost"
        ...
        <DialogContent className="bg-[#121214] border-slate-800 text-slate-100 max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-slate-400">
              Register a member who paid cash dues in person during committee
              meetings or callouts.
            </DialogDescription>
          </DialogHeader>
          ...
          <form
            onSubmit={async (e) => {
              ...
              try {
                ...
                setCashSuccessMsg(
                  `Recorded $${amountNum.toFixed(2)} cash dues payment for ${cashStudentName.trim()}!`,
                );
                setCashStudentName("");
                setCashPurdueEmail("");
                setTimeout(() => {
                ...
                setCashErrorMsg(
                  err?.message || "Failed to record cash dues payment.",
                );
              } finally {
              ...
              <div className="space-y-1.5">
                ...
                <Select value={cashSemester} onValueChange={setCashSemester}>
                  ...
                  <SelectContent className="bg-slate-900 border-slate-800 text-slate-100">
                    ...
                    <SelectItem value="Annual 2025-2026">
                      Annual 2025-2026
                    </SelectItem>
                  </SelectContent>
                </Select>
