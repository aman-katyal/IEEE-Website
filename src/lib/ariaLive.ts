 ...
 * Injects invisible, screen-reader accessible live regions for dynamic search/filter updates.
 */

let politeRegion: HTMLDivElement | null = null;
let assertiveRegion: HTMLDivElement | null = null;

function getOrCreateRegion(mode: 'polite' | 'assertive'): HTMLDivElement | null {
  ...
  if (mode === 'polite') politeRegion = el;
